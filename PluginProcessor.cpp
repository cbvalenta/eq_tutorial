...
{
    // setting spec values for preparation
    juce::dsp::ProcessSpec spec;
    spec.maximumBlockSize = samplesPerBlock;
    spec.numChannels = 1;
    spec.sampleRate = sampleRate;
    
    // prepare the 2 channels for audio playback
    leftChain.prepare(spec);
    rightChain.prepare(spec);
    
    // design and apply the low cut, peak, and high cut filters based on user-set parameters
    updateFilters();
}
...
void SimpleEQAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    // design and apply low cut, peak, and high cut filters to both channels of audio
    updateFilters();
    
    juce::dsp::AudioBlock<float> block(buffer);
    
    // isolate both channels of audio from the AudioBuffer
    auto leftBlock = block.getSingleChannelBlock(0);
    auto rightBlock = block.getSingleChannelBlock(1);
    
    // setting context values for processing based on both channels of audio
    juce::dsp::ProcessContextReplacing<float> leftContext(leftBlock);
    juce::dsp::ProcessContextReplacing<float> rightContext(rightBlock);
    
    // process both channels of audio
...
// load the user-set values for each parameter and store in a ChainSettings object
...
// design the peak filter based on user-set values and apply it to both channels of audio
...
// design the low cut filter based on user-set values and apply it to both channels of audio
...
// design the high cut filter based on user-set values and apply it to both channels of audio
...
// create a ParameterLayout object to keep track of parameter states
